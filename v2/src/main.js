...
import { BOARD, initBoard } from './game/boardGraph.js';
...
/* ================================================================
   Game objects
   ================================================================ */
// meshMap and boardGroup are assigned inside onStart() after initBoard() sets the mode
let meshMap   = null;
let boardGroup = null;
...
function getTileCenter(spaceId) {
  if (!meshMap) return new THREE.Vector3();
...
/* ================================================================
   Category legend — always visible, bottom-left above badge
   ================================================================ */
const _legend = document.createElement('div');
_legend.id = 'cat-legend';
_legend.innerHTML = `
  <div class="legend-body" id="legendBody">
    ...
    <div class="legend-row">
      <span class="legend-swatch" style="background:#fffacc;box-shadow:0 0 6px 2px #ffe56688"></span>
      <span class="legend-name">Roll Again</span>
    </div>
  </div>
`;
...
const setup = initSetup({
  onStart({ teams, edition, boardMode }) {
    // Configure board graph and build 3D board for chosen mode
    initBoard(boardMode ?? 'medium');
    const result = buildBoard(scene);
    meshMap    = result.meshMap;
    boardGroup = result.group;
    ...
    tokens.init(teams.length, teams.map(t => t.color));
    setup.hide();
    hud.show();
    startGame(teams, pool); // emits turnStart → handler shows banner, then enables roll
  },
});
...
// Keyboard shortcut: Space = roll die
