...
/* ============================================================
   ...
   Ring track: annular-sector tiles, 54 spaces, tessellating.
...
// Physical material — roll-again tiles (cream lacquer)
...
export function buildBoard(scene) {
  ...
  for (let cat = 0; cat < 6; cat++) {
    const a0  = ringAngle(cat * HQ_STEP) - Math.PI / HQ_STEP;
  ...
  /* ---- Ring tiles ---- */
  const aHalf = Math.PI / RING_SIZE; // half-angle per tile slot
  ...
  for (let i = 0; i < RING_SIZE; i++) {
    ...
    if (space.isRollAgain) {
      ...
      star.position.set(wPos.x, RING_H + 0.004, wPos.z);
      group.add(star);
    }
  }
