...
import { BOARD, findReachableSpaces, STARTING_SPACE } from './boardGraph.js';
...
export function onPlayerChoseDest(destId) {
  ...
  emit('clearReachable', {});
  emit('tokenMoveDirect', { teamIdx, fromId: state.tokenPositions[teamIdx], toId: destId });

  state.tokenPositions[teamIdx] = destId;
  state.phase = 'moving'; // wait for animation
}
