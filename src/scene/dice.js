...
/* ---------------------------------------------------------------
   Die rolls directly on the board surface.
   Floor sits just above all tile tops so the die never clips.
   Walls hug the inner edge of the brass rim.
   On roll(), the die is launched from the near (player) side of
   the board and tumbles across toward the center.
   --------------------------------------------------------------- */

const DIE_SIZE = 0.85;
const HALF     = DIE_SIZE / 2;

// Physics floor just above the tallest tile (hub top ≈ 0.27)
const FLOOR_Y  = 0.30;

// Die resting spot between outer ring tiles and brass rim (right side, visible)
const PARK_X = 7.8;
const PARK_Z = 2.8;

// Board walls — slightly inside the brass rim (r ≈ 9.4)
const WALL_R = 8.8;

// Body-frame face normals → pip values (standard Western die)
const FACE_NORMALS = [
...
export class DiceSystem {
  ...
  _buildPhysics() {
    ...
    // Left wall  x=−WALL_R  (normal +X)
    addPlane(-WALL_R, 1, 0,   0,  Math.PI / 2, 0);
    // Right wall x=+WALL_R  (normal −X)
    addPlane( WALL_R, 1, 0,   0, -Math.PI / 2, 0);
    // Back wall  z=−WALL_R  (normal +Z)
    addPlane(0, 1, -WALL_R,   0, 0, 0);
    // Front wall z=+WALL_R  (normal −Z)
    addPlane(0, 1,  WALL_R,   0, Math.PI, 0);
  }

  _buildDie() {
    const geo = new THREE.BoxGeometry(DIE_SIZE, DIE_SIZE, DIE_SIZE);
    const mat = new THREE.MeshStandardMaterial({ color: 0xf5ede0, roughness: 0.35, metalness: 0.05 });
    ...
    // Start parked at rest on the board felt (between ring and rim)
  ...
  _addPips() {
    const mat = new THREE.MeshStandardMaterial({ color: 0x15110d, roughness: 0.7 });
    ...
    const layouts = {
      ...
      6: [[-0.23,  0.23], [ 0.23,  0.23], [-0.23,  0   ], [ 0.23,  0   ], [-0.23, -0.23], [ 0.23, -0.23]],
    };
    const faces = [
  ...
  /** Throw die from the player's side — it tumbles across the board */
  roll(onResult) {
    this._onResult    = onResult;
    this._settling    = true;
    this._settleTimer = 0;

    // Launch from the near (camera-side) edge of the board, up high
    const launchX = (Math.random() - 0.5) * 8;
    this.body.position.set(launchX, 5.5, 8.0);

    // Strong inward throw (−Z = toward board center) with lateral spread
